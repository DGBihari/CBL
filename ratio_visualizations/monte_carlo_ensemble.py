...
# main SDE function with hybrid growth, spillover, suppression, seasonal, and noise components
def hybrid_ode(t, E_vec, alpha_perturb, B_perturb, beta_perturb):
    ...
    for i in range(n_pfas):
        ...
        growth = (alpha_vec[i] + alpha_perturb[i]) * E_vec_safe[i]
        ...
        if n_nb > 0:
            for j in neighbors:
                spillover += ((alpha_vec[j] + alpha_perturb[j]) / n_nb) * E_vec_safe[j]
...
# ensemble Monte Carlo runs
all_sols = []

for i in range(num_realizations):
    a_p = np.random.normal(0, 0.15 * alpha_vec)
    B_p = np.random.normal(0, 0.10 * B_vec)
    b_p = np.random.normal(0, 0.15 * beta_vec)

    ode_closure = lambda t, E_vec: hybrid_ode(t, E_vec, a_p, B_p, b_p)
...
ax.plot(t_years, mean_traj, color='#0072B2', linewidth=2.5, label='National Target Crime (Status Quo)')
ax.fill_between(t_years, mean_traj - 1.96 * std_traj, mean_traj + 1.96 * std_traj, color='#0072B2', alpha=0.2)

ax.set_title('Monte Carlo Spatio-Temporal SDE Forecast (2025-2028)', fontsize=14)
ax.set_xlabel('Year', fontsize=12)
ax.set_ylabel('Total Target Crimes', fontsize=12)
